...
>(({ defaultOpen = true, open: openProp, onOpenChange, className, style, children, ...props }, ref) => {
  ...
  // This is a simple example, you can use any approach here
  const [open, setOpen] = React.useState(openProp ?? defaultOpen)
  const isMobile = React.useState(false)[0] // Simplified, you might want to use a hook
...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  return (
    ...
    >
      <div
        data-sidebar="sidebar"
        className="flex h-full w-full flex-col bg-[var(--sidebar-background)] text-[var(--sidebar-foreground)] group-data-[collapsible=icon]:bg-[var(--sidebar-background)] group-data-[side=right]:border-l group-data-[side=left]:border-r border-[var(--sidebar-border)]"
      >
        {children}
      </div>
    </div>
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 overflow-auto group-data-[collapsible=icon]:overflow-hidden group-data-[collapsible=icon]:overflow-visible",
        className
      )}
      {...props}
    />
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Skeleton component would be imported, using a simple div for now
  return (
    <div
      ref={ref}
      data-sidebar="menu-skeleton"
      className={cn("rounded-md h-8 bg-sidebar-primary/10", className)}
      {...props}
    />
  )
})
